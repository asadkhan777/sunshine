...
public class FetchWeatherTask extends AsyncTask<String, Void, Void> {
    ...
    long addLocation(String locationSetting, String cityName, double lat, double lon) {
        ...
        Cursor loco = mContext.getContentResolver().query(WeatherContract.LocationEntry.CONTENT_URI,
                new String[]{WeatherContract.LocationEntry._ID},
                WeatherContract.LocationEntry.COLUMN_LOCATION_SETTING + " = ?",
                new String[]{locationSetting},
                null);

        if( loco != null && loco.moveToFirst() ){
            int locationIDIndex = loco.getColumnIndex(WeatherContract.LocationEntry._ID);
            location_id = loco.getLong(locationIDIndex);
            loco.close();
        } else {
            if (loco != null ) {
                loco.close();
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if ( cVVector.size() > 0 ) {
                ...
                ContentValues[] cv_array = new ContentValues[cVVector.size()];
                cVVector.toArray(cv_array);
                inserted = mContext.getContentResolver().bulkInsert(WeatherEntry.CONTENT_URI, cv_array);
            }
